...
vector<double> Wave::SystemOfEquations(vector<INT16>canal) {
	...
	for (i = 0; i < ammount_of_samples / 2; i++)
	...
	for (int b = 5; b <= 24; b++) {
		...
		for (size_t i = 0; i < ammount_of_samples / 2; i++) {
			...
			if (i <= r)
				...
				for (size_t j = 0; j < r; j++)
					sumError += decoder.at(j) * canal.at(i - j);
				if (sumError > pow(2, 15) - 1)
					sumError = pow(2, 15) - 1;
				else if (sumError < -pow(2, 15))
					sumError = -pow(2, 15);
		...
		Lsr = entro_minus(entropiaEPS) + ((32 + (r - 1) * (b + 1) + 10) / ammount_of_samples);

		if (minLsr > Lsr) {
			minLsr = Lsr;
			diagramBit = b;
			entropia = entro_minus(entropiaEPS);
		}
